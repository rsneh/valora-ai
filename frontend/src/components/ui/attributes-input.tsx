...
const AttributesInput = ({ value = {}, placeholder, allowAddition = false, onChange }: AttributesInputProps) => {
          ...
          attributeEntries.map(([key, value]) => (
            <div key={key} className="flex gap-2">
              <div className="w-32 md:w-40 flex-none bg-gray-50 px-3 py-2 rounded-md">
                <span className="font-medium text-gray-800">{t(`attributeInput.${key.toLowerCase()}`)}</span>
              </div>
